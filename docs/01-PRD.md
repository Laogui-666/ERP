...
| **M7: 门户化改造** | **Week 16-17** | **统一登录入口、Portal首页（广告Banner+6大工具入口+推荐内容流）、门户布局（底部4Tab）、ERP入口集成（账号面板）、6大工具模块骨架（资讯/行程/申请表/评估/翻译/证明文件）** | **P0** | **🔲 待开发** |

> **架构决策**：M7 采用"不动存量、纯增量"方案 — ERP系统（141文件）零改动，新增门户层（~16文件）独立路由 `/portal/*`，仅改3个现有文件（page.tsx + login/page.tsx + middleware.ts）。详见 `08-architecture-redesign.md V3.0`。

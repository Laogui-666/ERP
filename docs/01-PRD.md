...
| M3: 文件与客户端 | Week 6-8 | 阿里云OSS预签名直传、客户上传组件(A类)、材料下载面板(B类)、客户详情页、确认提交、出签反馈、通知中心、Socket.io客户端、个人中心、通知闭环(9节点) | P0 |
| M4: 实时通信 | Week 9-10 | Socket.io通知、站内信、SMS预留接口 | P1 |
| **M5: 多申请人+数据看板** | **Week 11-13** | **Applicant子表(✅)、多申请人工作流(✅)、财务字段(✅)、数据看板(✅)、Excel导出(✅)、历史数据迁移(⬜)** | **P0** |
| M6: 行业优化 | Week 14-15 | 签证模板库、智能检查、批量操作、订单模板 | P2 |

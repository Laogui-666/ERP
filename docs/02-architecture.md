...
| 路径 | 权限 | 说明 |
|---|---|---|
| `/` | 公开可浏览 | C 端产品首页（Airbnb 风格） |
| `/services` | 公开可浏览 | 签证服务列表 |
| `/tools/*` | 公开可浏览 | 6 大智能工具 |
| `/orders` | 已登录(CUSTOMER) | 我的订单 |
| `/profile` | 已登录 | 个人中心 + ERP 入口 |
...
| POST | `/api/orders/[id]/submit` | 客户确认提交（基于有文件判断→REVIEWING→通知资料员 DOCS_SUBMITTED） | Lv9 (CUSTOMER) | ✅ M3 批次3 |

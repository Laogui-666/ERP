...
> **当前阶段**: M1 ✅ → M2 ✅ → M3 ✅ → M4 ✅ → M5 ✅ → M6 ✅ → **M7 Phase 0-5 ✅** → **架构合规修复 ✅** → **M7 Phase 5 工具模块内容 ✅ 完成**
...
| **项目代号** | VISA-ERP |
| **所属公司** | 华夏签证 |
| **产品定位: 面向 C 端用户的综合签证服务平台（含 ERP 后台子系统） |
...
| **代码统计** | 166 源文件 / ~20,770 行 / 57 API 路由 / 28 页面 / 32 组件 / 5 测试文件(91 用例) |
...
| 12 | ChatRoom | erp_chat_rooms | 聊天会话（M4新增） |
| 13 | ChatMessage | erp_chat_messages | 聊天消息（M4新增） |
| 14 | ChatRead | erp_chat_reads | 已读回执（M4新增） |
| 15 | NewsArticle | erp_news_articles | 签证资讯（M7新增） |
| 16 | Itinerary | erp_itineraries | 行程助手（M7新增） |
| 17 | FormTemplate | erp_form_templates | 申请表模板（M7新增） |
| 18 | FormRecord | erp_form_records | 申请表记录（M7新增） |
| 19 | VisaAssessment | erp_visa_assessments | 签证评估（M7新增） |
| 20 | TranslationRequest | erp_translation_requests | 翻译请求（M7新增） |
| 21 | DocHelperTemplate | erp_doc_helper_templates | 证明文件模板（M7新增） |
| 22 | GeneratedDocument | erp_generated_documents | 已生成文档（M7新增） |

> 以上 14 张表已通过 Prisma Schema 定义，Chat 表需在 ECS 上执行 `npx prisma migrate deploy` 创建。
...
> **⚠️ M7 模块化重构预告**：当前目录结构为扁平化布局（Phase 0 之前）。M7 Phase 0 将执行分层模块化重构：
> - `src/shared/` — 纯公共基础设施（prisma/auth/rbac/utils/oss/socket + UI组件 + 共享类型/Store/Hook）
> - `src/modules/erp/` — ERP 业务模块（transition/desensitize/events/chat-system + 订单/资料/分析/聊天组件 + ERP专属Hook/Store/类型）
> - `src/components/portal/` — 门户组件（首页/顶栏/工具网格/统计/目的地轮播）
> - `src/app/portal/` — 门户路由
> - tsconfig 路径别名：`@shared/*`、`@erp/*`、`@/*`（兼容）
> - 完整迁移映射表见 `08-architecture-redesign.md` V4.0
>
> 以下目录地图反映当前（重构前）的文件位置。
...
| `auth/change-password/route.ts` | POST | `/api/auth/change-password` | ✅ | 修改密码（旧密码校验+PRD合规规则+空密码检测，M3批次5） |
...
| `orders/[id]/submit/route.ts` | POST | `/api/orders/[id]/submit` | ✅ | 客户确认提交资料 + DOCS_SUBMITTED通知资料员 + Socket推送（M3 批次3） |
...
| `chat/rooms/route.ts` | GET | `/api/chat/rooms` | ✅ | 会话列表（M4批次2） |
| `chat/rooms/[orderId]/route.ts` | GET | `/api/chat/rooms/[orderId]` | ✅ | 获取/创建会话（M4批次2） |
| `chat/rooms/[orderId]/messages/route.ts` | GET/POST | `/api/chat/rooms/[orderId]/messages` | ✅ | 消息分页+发送（M4批次2） |
| `chat/rooms/[orderId]/read/route.ts` | POST | `/api/chat/rooms/[orderId]/read` | ✅ | 标记已读（M4批次2） |
| `documents/presign/route.ts` | POST | `/api/documents/presign` | ✅ | 预签名直传 URL（M3 批次2） |
| `documents/confirm/route.ts` | POST | `/api/documents/confirm` | ✅ | 上传确认写库 + MIME 修正（M3 批次2） |
| `documents/files/[id]/route.ts` | DELETE | `/api/documents/files/[id]` | ✅ | 文件级删除（M3 批次2） |
| `shop/sync/route.ts` | POST | `/api/shop/sync` | ⚠️ | 店铺订单同步（501预留） |
| `shop/webhook/route.ts` | POST | `/api/shop/webhook` | ⚠️ | 店铺订单回调（501预留） |
| `sms/send/route.ts` | POST | `/api/sms/send` | ⚠️ | 发送短信（501预留） |
| `sms/templates/route.ts` | GET | `/api/sms/templates` | ⚠️ | 短信模板列表（501预留） |
| `templates/route.ts` | GET+POST | `/api/templates` | ✅ | 签证模板列表+创建（M6） |
| `templates/[id]/route.ts` | GET+PATCH+DELETE | `/api/templates/[id]` | ✅ | 模板详情/更新/删除（M6） |
| `templates/apply/route.ts` | POST | `/api/templates/apply` | ✅ | 模板应用到订单（M6） |
| `orders/[id]/check/route.ts` | GET | `/api/orders/[id]/check` | ✅ | 智能资料检查（M6） |
| `orders/batch/route.ts` | POST | `/api/orders/batch` | ✅ | 批量操作（M6） |
| `order-templates/route.ts` | GET+POST | `/api/order-templates` | ✅ | 快速录入+快速创建订单（M6） |
| `companies/me/route.ts` | GET+PATCH | `/api/companies/me` | ✅ | 公司信息（M6） |
...
| `admin/templates/page.tsx` | `/admin/templates` | ✅ | 签证模板库（CRUD+展开+系统模板标记） |
| `admin/team/page.tsx` | `/admin/team` | ✅ | 团队管理（员工列表+创建+角色标签+部门筛选） |
| `admin/analytics/page.tsx` | `/admin/analytics` | ✅ | 数据统计看板（M5 完成） |
| `admin/settings/page.tsx` | `/admin/settings` | ✅ | 系统设置（公司信息编辑） |
| `admin/layout.tsx` | — | ✅ | 管理端布局（侧边栏+顶栏） |
| `customer/orders/page.tsx` | `/customer/orders` | ✅ | 客户订单页（Link可点击+待办提示+多人标记） |
| `customer/orders/[id]/loading.tsx` | — | ✅ | 客户订单详情页加载骨架屏 |
| `customer/orders/[id]/page.tsx` | `/customer/orders/[id]` | ✅ | 客户订单详情页（时间线+上传+确认提交+材料下载+出签反馈+订单信息+操作记录，M3 批次3） |
| `customer/notifications/page.tsx` | `/customer/notifications` | ✅ | 通知中心（分页加载+未读标记+类型图标+点击跳转+全部已读，M3 批次4） |
| `customer/profile/page.tsx` | `/customer/profile` | ✅ | 个人中心（用户信息+修改密码+订单入口+退出登录，M3 批次5） |
| `customer/layout.tsx` | — | ✅ | 客户端布局（3Tab路由+Socket实时角标+断连轮询fallback，M3 批次4改造） |
| `page.tsx` | `/` | ✅ | 首页（自动重定向） |
| `layout.tsx` | — | ✅ | 全局布局 |
...
| `documents/customer-upload.tsx` | ✅ | 客户上传组件（预签名三步+拍照+删除+分组，M3 批次2） |
| `documents/material-panel.tsx` | ✅ | 签证材料面板（上传+版本分组+文件预览+HEIC支持） |
| `orders/material-checklist.tsx` | ✅ | B类材料下载面板（MAKING_MATERIALS制作中提示+PENDING_DELIVERY+文件预览+下载，M3 批次3） |
| `notifications/notification-bell.tsx` | ✅ | 通知铃铛组件 |
| `chat/chat-message.tsx` | ✅ | 单条消息组件（TEXT/IMAGE/FILE/SYSTEM+角色标签+头像，M4批次4） |
| `chat/chat-message-list.tsx` | ✅ | 消息列表（自动滚底/加载更多/滚动位置保持/typing指示器，M4批次4） |
| `chat/chat-input.tsx` | ✅ | 输入区（文本/文件/图片上传+OSS预签名/字数限制/Enter发送，M4批次4） |
| `chat/chat-panel.tsx` | ✅ | 聊天主面板（组装列表+输入+typing+自动已读+图片灯箱，M4批次4） |
| `chat/chat-room-list.tsx` | ✅ | 顶栏会话列表下拉（未读数/实时刷新/点击跳转，M4批次4） |
...
| `api-client.ts` | ✅ | 封装 fetch：401 自动刷新 Token + 重试（防抖多请求只刷一次） |
...
| `events.ts` | ✅ | 事件总线 + 状态变更通知处理器（11个状态标签含PARTIAL）——统一引用 ORDER_STATUS_LABELS，无重复定义 |
| `socket.ts` | ✅ | Socket.io 服务端（JwtPayload认证+公司/用户/订单房间+chat:join/leave/typing/mark-read+auto-join+disconnect清理+emitToRoom，M3/M4） |
| `oss.ts` | ✅ | 阿里云 OSS 客户端（上传/删除/签名URL/预签名直传/getOssClient单例导出） |
| `file-types.ts` | ✅ | 文件类型白名单常量（ALLOWED_FILE_TYPES + MAX_FILE_SIZE，M3 批次2） |
| `logger.ts` | ✅ | 统一错误日志收集（结构化输出 + handleApiError 统一处理） |
| `notification-icons.ts` | ✅ | 通知类型图标+路由映射共享常量（M3 批次4） |
| `chat-system.ts` | ✅ | 聊天系统消息工具（sendSystemMessage+archiveChatRoom，M4 批次1） |

### 3.5 单元测试 (`src/lib/__tests__/`)

| 文件 | 用例数 | 说明 |
|---|:---:|---|
| `utils.test.ts` | 22 | 财务计算/订单号生成/日期格式化/cn/pick/omit |
| `rbac.test.ts` | 16 | 权限矩阵/路由权限/数据范围过滤 |
| `desensitize.test.ts` | 21 | 手机/护照/身份证/邮箱/银行卡脱敏 + 订单数据脱敏 |
| `transition.test.ts` | 15 | 状态机规则完整性/各角色可用流转/终态无流转 |

### 3.6 服务层 (`src/services/`) — 待建

> 服务层目录尚未创建。当前所有业务逻辑直接写在 API Route 中。
> 未来 M6 阶段如需统一服务层，再创建此目录。
...
| `chat-store.ts` | ✅ | 聊天状态（会话列表/消息缓存/分页/发送/实时添加，M4批次4） |
...
| `use-socket-client.ts` | ✅ | Socket 客户端 Hook（单例+回调注册表+Cookie认证+isConnected+disconnect+joinRoom/leaveRoom/sendTyping/socketMarkRead，M3/M4） |
| `use-chat.ts` | ✅ | 聊天 Hook（Socket注册/加载/发送/typing防抖/标记已读，M4批次4） |
...
| `chat.ts` | ✅ | ChatRoom/ChatMessageItem/SendMessagePayload/ChatRoomSummary + Socket事件payload（M4批次1） |
...
| POST | `/api/auth/change-password` | 已登录 | ✅ | 修改密码（旧密码校验+PRD合规密码规则+空密码检测） |
...
| POST | `/api/shop/sync` | Lv2-4 | ⚠️ | 店铺订单同步（501预留） |
| POST | `/api/shop/webhook` | 公开 | ⚠️ | 店铺订单回调（501预留） |
| POST | `/api/sms/send` | — | ⚠️ | SMS 发送（501预留） |
| GET | `/api/sms/templates` | — | ⚠️ | SMS 模板（501预留） |
| GET | `/api/templates` | Lv2,5-6 | ✅ | 签证模板列表（M6） |
| POST | `/api/templates` | Lv2,5-6 | ✅ | 创建签证模板（M6） |
| GET | `/api/templates/[id]` | Lv2,5-6 | ✅ | 模板详情（M6） |
| PATCH | `/api/templates/[id]` | Lv2,5-6 | ✅ | 更新模板（M6） |
| DELETE | `/api/templates/[id]` | Lv2,5-6 | ✅ | 删除模板（M6） |
| POST | `/api/templates/apply` | Lv5-7 | ✅ | 模板应用到订单（M6） |
| GET | `/api/orders/[id]/check` | 有权限 | ✅ | 智能资料检查（M6） |
| POST | `/api/orders/batch` | Lv2-5 | ✅ | 批量操作（M6） |
| GET | `/api/order-templates` | Lv2-4 | ✅ | 快速录入数据（M6） |
| POST | `/api/order-templates` | Lv2-4 | ✅ | 快速创建订单（M6） |
| GET | `/api/companies/me` | 已登录 | ✅ | 当前公司信息（M6） |
| PATCH | `/api/companies/me` | Lv2 | ✅ | 更新公司信息（M6） |
...
| 方法 | 路径 | 权限 | 状态 | 说明 |
|---|---|---|:---:|---|
| GET | `/api/chat/rooms` | Lv2-7,9 | ✅ | 会话列表（原生SQL+未读数） |
| GET | `/api/chat/rooms/[orderId]` | 有权限 | ✅ | 获取/创建会话（upsert） |
| GET | `/api/chat/rooms/[orderId]/messages` | 有权限 | ✅ | 消息分页（复合游标） |
| POST | `/api/chat/rooms/[orderId]/messages` | Lv2-7,9 | ✅ | 发送消息（事务+Socket推送） |
| POST | `/api/chat/rooms/[orderId]/read` | 有权限 | ✅ | 标记已读（upsert+Socket回执） |
...
| **M5：资料面板分组** | **✅** | **批次8：applicantCount>1 时按人分组展示 + 进度统计** |
| **M3：Tab 导航路由化** | ✅ | 客户端3个Tab（订单/消息/我的）+ 未读角标轮询 |
| **M3：订单列表增强** | ✅ | Link包裹可点击+待办提示+多人标记 |
| **M3：状态时间线组件** | ✅ | 6步进度条+完成时间+终态展示 |
| **M3：权限补全** | ✅ | CUSTOMER加delete+transition |
| **M3：预签名直传 API** | ✅ | POST /api/documents/presign（文件类型白名单+companyId校验） |
| **M3：上传确认 API** | ✅ | POST /api/documents/confirm（事务写库+条件状态更新+OSS MIME修正） |
| **M3：文件删除 API** | ✅ | DELETE /api/documents/files/[id]（OSS+DB删除+CUSTOMER仅删自己+状态回退） |
| **M3：通知类型扩展** | ✅ | DOCS_SUBMITTED 枚举值（Schema+TypeScript） |
| **M3：客户上传组件** | ✅ | CustomerUpload（预签名三步+XHR进度条+拍照+文件预览+删除+多人分组） |
| **M3：Socket Cookie 认证** | ✅ | socket.ts io.use() 增加 Cookie fallback 解析 |
| **M3：发送清单通知客户** | ✅ | POST documents 创建 DOC_REVIEWED 通知 + Socket 推送 |
| **M3：客户订单详情页** | ✅ | A类资料上传 + B类材料下载 + 确认提交 + 出签反馈（M3 批次3） |
| **M3：客户确认提交** | ✅ | POST /api/orders/[id]/submit + DOCS_SUBMITTED 通知 + 幂等处理（M3 批次3） |
| **M3：B类材料面板** | ✅ | MaterialChecklist（MAKING_MATERIALS制作中提示+PENDING_DELIVERY+文件列表+下载，M3 批次3） |
| **M3：通知中心** | ✅ | 通知列表+分页加载+未读标记+类型图标+点击跳转订单+全部已读（M3 批次4） |
| **M3：Socket.io 客户端** | ✅ | use-socket-client Hook（单例+Cookie认证+isConnected+disconnect+visibilitychange重连，M3 批次4） |
| **M3：通知图标共享** | ✅ | notification-icons.ts 共享常量（9种类型图标+路由映射，M3 批次4） |
| **M3：Tab 角标实时化** | ✅ | layout.tsx 集成 Socket + isConnected 控制轮询 fallback（M3 批次4） |
| **M3：个人中心** | ✅ | 信息展示 + 修改密码入口 + 订单入口 + 退出登录（M3 批次5） |
| **M3：修改密码 API** | ✅ | POST /api/auth/change-password + 旧密码校验 + 空密码检测 + PRD合规密码规则（M3 批次5） |
| **M3：通知闭环修复** | ✅ | 4个API路由补充Socket实时推送(documents/[id] PATCH/materials POST/cancel POST/reassign POST) + 全链路9节点验证通过（M3 批次6） |
| **M3：管理端增强** | ✅ | DocumentPanel文件删除集成(canDeleteFile权限+FilePreview onDelete) + 批量上传3路并发池优化（M3 批次6） |
| **工作流验证：通知链补全** | **✅** | **documents/[id] PATCH 操作员审核时同步通知资料员（REJECTED/SUPPLEMENT/APPROVED 含进度统计）** |
| **工作流验证：材料打回备注** | **✅** | **PENDING_DELIVERY→MAKING_MATERIALS 备注必填 + 动态 placeholder** |
| **工作流验证：复审文案** | **✅** | **根据 orderLogs 判断"提交复审" vs "提交审核"** |
| **工作流验证：签证类型反馈** | **✅** | **电子签客户不可反馈(显示专员提示)/申根签标记/其他签客户可反馈** |
| **工作流验证：全部合格提示** | **✅** | **客户端资料全部 APPROVED 时绿色完成提示** |
| **Shop API 预留** | **⚠️** | **/api/shop/sync + /api/shop/webhook（501预留，支持网店订单同步）** |
| **SMS API 预留** | **⚠️** | **/api/sms/send + /api/sms/templates（501预留）** |
| **M6：签证模板库** | **✅** | **模板CRUD + 系统预置 + 一键应用到订单 + 资料面板集成** |
| **M6：智能资料检查** | **✅** | **护照有效期/资料齐全/多人完整/签证类型推荐/财务异常 5 项检查** |
| **M6：批量操作** | **✅** | **apply_template/transition/cancel 三种批量操作，最多 50 单** |
| **M6：签证分类流程差异化** | **✅** | **电子/贴纸/申根/指纹 4 类流程提示卡片（管理端+客户端）** |
| **M6：订单模板快速录入** | **✅** | **模板快速填充 + 老客户自动填充 + 自动应用资料清单** |
| **M6：团队管理** | **✅** | **员工列表+创建表单+角色标签+部门筛选** |
| **M6：系统设置** | **✅** | **公司信息编辑（名称/电话/邮箱/地址）** |
...
M3 ████████████████████████████████████████ ████ ✅ 100%  Week 6-8 (全部8批次完成，含批次7验收)
M4 ████████████████████████████████████████ ████ ✅ 100%  Week 9-10 (全部5批次完成，133源文件/~15300行)
M5 ████████████████████████████████████████ ████ ✅ 100%  Week 11-13 (全部8批次完成)
M6 ████████████████████████████████████████ ████ ✅ 100%  Week 14-15 (签证模板+智能检查+批量操作+流程差异化+订单快录)
M7 ████████████████████████████████████████ ████ ✅ 100%  Phase 0-5 全部完成（8Model+7API+6页面） — V5.1
M7-P0 ████████████████████████████████████████ ████ ✅ 100%  Phase 0 目录重构完成（2026-04-01）
M7-P5 ████████████████████████████████████████ ████ ✅ 100%  Phase 5 工具模块内容开发完成（2026-04-02）
M8 ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░     🔲 待开发  C 端平台化改造（品牌统一+首页重写+导航重写+ERP入口隐藏+工具UI完善）
TD ████████████████████████████████████████ ████ ✅ 100%  技术债务修复完成
WV ████████████████████████████████████████ ████ ✅ 100%  工作流完整性验证完成（2026-03-28）
```

### M4 实时通信 — 批次计划（5 批次 / ~22h）

| 批次 | 内容 | 状态 |
|---|---|:---:|
| 批次 1 | 数据层：Schema + 迁移 + 类型 + 种子 + RBAC + JwtPayload + ChatRoom 创建 + 系统消息工具 + 事件总线 | ✅ 完成 |
| 批次 2 | 聊天 API：会话列表 + 消息 CRUD + 已读回执 + presign/confirm 扩展 | ✅ 完成 |
| 批次 3 | Socket.io：emitToRoom + 聊天事件(chat:join/leave/typing/mark-read) + 回调注册表 + auto-join + disconnect 清理 | ✅ 完成 |
| 批次 4 | 前端 UI：ChatStore + useChat + ChatPanel + ChatInput + ChatMessageList + ChatRoomList + 顶栏集成 + 管理端/客户端浮动按钮 | ✅ 完成 |
| 批次 5 | 验收 + 优化 + 测试 + 离线通知 + 文档更新 | ✅ 完成 |
...
> 完成于 2026-03-30 | 140 源文件 / ~18,500 行 / 45 API 路由 / 93 测试用例

| 任务 | 文件 | 状态 |
|---|---|:---:|
| 签证模板库 CRUD | `admin/templates/page.tsx` + `api/templates/` (3 路由) | ✅ |
| 模板应用到订单 | `api/templates/apply/route.ts` | ✅ |
| 智能资料检查 | `api/orders/[id]/check/route.ts`（护照/齐全/多人/推荐/财务 5 项检查） | ✅ |
| 批量操作 | `api/orders/batch/route.ts`（apply_template/transition/cancel，最多 50 单） | ✅ |
| 签证分类流程差异化 | `admin/orders/[id]/page.tsx` VisaFlowHints 组件（电子/贴纸/申根/指纹 4 类提示） | ✅ |
| 订单模板快速录入 | `api/order-templates/route.ts` + `admin/orders/page.tsx` 快速录入面板 | ✅ |
| 老客户自动填充 | 同上，最近 10 个客户一键填入历史信息 | ✅ |
| 团队管理 | `admin/team/page.tsx`（员工列表+创建表单） | ✅ |
| 系统设置 | `admin/settings/page.tsx` + `api/companies/me/route.ts` | ✅ |
| 订单管理增强 | `admin/orders/page.tsx` 批量选择+操作栏 | ✅ |
| 资料面板模板集成 | `document-panel.tsx` 从模板库选择按钮 | ✅ |
| **验收** | **tsc 0 错误 / build 0 警告 / test 93 通过 / as any 0 / console.log 0 / TODO 0** | ✅ |
...
| 2026-03-22 | V7.2 | M5批次8完成→M5全部完成✅：document-panel.tsx资料面板多人分组展示。①新增applicantCount+applicants props ②applicantCount>1时需求均匀分配到各申请人组 ③每组显示申请人头像+姓名+进度(已合格/总数) ④全部合格组显示绿色标记 ⑤单人订单保持平铺展示不变 ⑥订单详情页传递新props；tsc 0错误；build通过0 Warning |
| 2026-03-22 | V8.0 | 三轮全面深度审查+14项修复+API客户端封装：**第一轮**(6项) ①seed.ts手机号合规 ②events.ts去重复STATUS_LABELS→统一导入ORDER_STATUS_LABELS ③auth/me lastLoginAt统一ISO格式 ④客户状态流程5→6步(加入PENDING_CONNECTION) ⑤analytics/overview全量加载→3次原生SQL聚合 ⑥services目录确认不存在→文档修正；**第二轮**(1项) ⑦applicant-card拒签弹窗父容器加relative定位；**第三轮**(7项) ⑧新建src/lib/api-client.ts封装fetch：401自动刷新Token+重试+防抖 ⑨全部25处认证API调用替换为apiFetch ⑩5处`<a>`→`<Link>`内部导航修复(login/register/reset-password/orders/workspace) ⑪9个文件`'use client'`首行修复(避免Server Component编译错误)；源文件90/10247行；tsc 0错误；build通过；文档全部更新 |
| 2026-03-22 | V9.0 | 框架升级+技术债务全面修复：**框架升级** Next.js 14.2.18→15.5.14 + React 18.3.1→19.2.4；10个API路由params改为Promise类型；next.config.js serverComponentsExternalPackages→serverExternalPackages；安装graceful-fs消除ali-oss编译警告；**错误边界** 新增global-error.tsx/admin/error.tsx/customer/error.tsx/not-found.tsx/admin/loading.tsx共5个页面级错误兜底；**统一日志** 新建src/lib/logger.ts(结构化错误日志+handleApiError统一处理)集成到events.ts和documents/[id]；**分页修复** users/departments API添加page/pageSize/skip/take；**Cron幂等性** 改为精确到日期级别去重(当天0点)；**部署配置** ecosystem.config.json(PM2)+deploy/nginx-erp.conf(Nginx反向代理+WebSocket+HTTPS)+scripts/deploy.sh(自动化部署)+.env.example(环境变量模板)+scripts/db-backup.sh(数据库备份)；**单元测试** vitest框架+4个测试文件74个用例(utils/rbac/desensitize/transition)；**代码规范** customer/orders/page.tsx修正'use client'首行；源文件100/11183行；tsc 0错误；build 0警告；74 tests pass |
| 2026-03-22 | V10.0 | 两轮全面深度审查+7项修复+3项优化：**第一轮**(7项) ①DATABASE_URL密码缺失(.env.local) ②Bootstrap脚本heredoc密码替换失败(ERP-BOOTSTRAP.sh) ③订单列表缺少createdBy字段(客服数据过滤依赖) ④Socket.io实时通知未触发(eventBus→emitToUser) ⑤PRD成功色值拼写错误(#7FA seventeen87A→#7FA87A) ⑥DocumentFile.label缺少@map ⑦订单列表缺少扩展字段(externalOrderNo/visaCategory/applicantCount)；**第二轮**(全量逐文件审查100个源文件) 确认：TypeScript严格模式合规/无as any/Prisma??null合规/erp_前缀完整/@map完整/'use client'首行/Link导航/API Zod校验/权限检查/租户隔离/脱敏处理/错误边界/74测试通过/构建0警告；**优化建议**(3项非阻塞) ①支付方式值对齐(创建表单中文vs导入脚本标准化代码) ②登录API补全字段(departmentId等) ③事件Socket推送扩展(ORDER_CREATED/MATERIAL_UPLOADED等)；源文件100/11183行/30 API路由/15页面/22组件/4测试文件74用例；tsc 0错误；build 0警告；74 tests pass |
| 2026-03-23 | V11.0 | M3 全知手册定稿+文档全面更新：**M3计划**两轮深度分析（逐文件审查100个源文件+30个API路由+工作流文档+客户材料清单），发现5个关键缺口（documents POST不通知客户/确认提交无API/CUSTOMER缺delete权限/Socket无法读HttpOnly Cookie/缺DOCS_SUBMITTED类型），制定8批次22h开发计划含22个子任务；**文档更新** 01-PRD→V6.0(里程碑描述)、02-architecture→V6.0(新增M3 API模块+Socket Cookie改造)、03-project-status→V11.0(里程碑+实现状态+变更日志)、04-dev-standards→V5.0、05-M5-dev-plan→V5.0、06-M3-dev-plan→V3.0终版(1129行18章节22子任务) |
| 2026-03-23 | V12.0 | M3 全知手册V4.0全面重写+文档全面更新：**第三轮深度分析**（逐阶段核对实际工作流文档5阶段20+步骤+逐API比对），发现7个具体问题（追加需求项通知缺口/客户端出签反馈UI缺失/材料上传通知冗余/操作员反馈语义模糊/确认提交逻辑需修正/B类材料制作中提示/个人中心订单入口）；**M3优化**工作量22h→25h，新增6项优化（M3-9确认提交逻辑基于有文件判断/M3-18覆盖追加场景/M3-11出签反馈UI/M3-10制作中提示/M3-16订单入口/通知去重）；**架构决策**扩展至10项（新增确认提交逻辑修正）；**验收标准**扩展至12个功能点+15步端到端测试+9项全局检查；**文档更新** 01-PRD→V7.0(里程碑M3交付物细化)、02-architecture→V7.0(M3 API模块补充状态列+Socket改造)、03-project-status→V12.0(里程碑+变更日志)、04-dev-standards→V6.0、05-M5-dev-plan→V6.0、06-M3-dev-plan→V4.0终版(1487行18章节25h/8批次22子任务) |
| 2026-03-23 | V13.0 | M3 批次1完成：**M3-1** Tab导航路由化(customer/layout.tsx 3个Tab+未读角标轮询)；**M3-2** 订单列表增强(customer/orders/page.tsx Link包裹+待办提示COLLECTING_DOCS/PENDING_DELIVERY/DELIVERED+多人👥标记)；**M3-3** 状态时间线组件(新建status-timeline.tsx 6步进度条+完成时间+终态展示+pulse动画)；**M3-4** 权限补全(rbac.ts CUSTOMER加delete+transition)；**额外** customer/orders/[id]/loading.tsx骨架屏、server.ts console.log→process.stdout、documents/[id]/route.ts console.error→logApiError；源文件100→102/11257→11518行/组件22→23；tsc 0错误；build 0警告；**文档更新** 全部6份文档版本号+日期更新、03里程碑进度0%→12.5%、06批次1标记完成 |
| 2026-03-25 | V14.0 | M3 批次2深度分析完成：**两轮审查**逐文件审查20个关键文件(tsc 0错误确认)；**发现7个缺口**P0×3(Socket Cookie认证失败/发送清单客户无通知/缺DOCS_SUBMITTED类型)、P1×2(presigned URL缺Content-Type修正/presign缺文件类型校验)、P2×2(OSS CORS/confirm状态条件更新)；**批次2任务确认**7项子任务+1项验收(M3-5预签名/M3-6确认/M3-7删除/M3-19通知类型/M3-8客户上传组件/M3-23 Socket Cookie/M3-24清单通知)，预估4.5h；**文档更新** 01-PRD→V9.0、02-architecture→V9.0、03-project-status→V14.0、04-dev-standards→V8.0、05-M5-dev-plan→V8.0、06-M3-dev-plan→V6.0全知重写 |
| 2026-03-25 | V15.0 | M3 批次2全部完成✅（7/7子任务）：**新建5文件** presign/route.ts(预签名直传)、confirm/route.ts(上传确认+MIME修正)、files/[id]/route.ts(文件删除)、customer-upload.tsx(客户上传组件)、file-types.ts(文件类型白名单)；**修改6文件** schema.prisma(+DOCS_SUBMITTED枚举)、types/order.ts(+DOCS_SUBMITTED)、socket.ts(Cookie认证改造)、oss.ts(导出getOssClient单例)、documents/route.ts POST(+通知客户)、upload/route.ts(引用共享file-types)；**深度审查修复4项** P0 ossKey路径结构验证(startsWith替代includes)、P1 OSS单例复用、P1 ALLOWED_TYPES提取共享常量、P2批量上传统一刷新；**质量验收** tsc 0错误/build 0警告/test 74通过/as any 0/console.log 0/TODO 0/use client首行全对；**源文件** 107个/33 API路由/24组件 |
| 2026-03-25 | V16.0 | M3 批次3全部完成✅（3/3子任务）：**新建3文件** submit/route.ts(客户确认提交API-117行)、material-checklist.tsx(B类材料面板-93行)、customer/orders/[id]/page.tsx(客户订单详情页-352行)；**M3-9 submit API** customerId校验+COLLECTING_DOCS状态校验+有文件才可提交+幂等处理(REVIEWING不重复更新)+事务内有文件→REVIEWING+操作日志+DOCS_SUBMITTED通知资料员+Socket推送；**M3-10 MaterialChecklist** MAKING_MATERIALS显示制作中提示+动画/PENDING_DELIVERY+文件列表+预览+下载(用DB ossUrl签名链接)/其他状态不显示；**M3-11 客户详情页** 返回+订单号+状态徽章/StatusTimeline竖向时间线/CustomerUpload上传+确认提交按钮/出签反馈(DELIVERED状态可点确认/拒签)/MaterialChecklist材料面板/订单信息/操作记录；**质量验收** tsc 0错误/build 0警告0错误/test 74通过/as any 0/console.log 0/TODO 0/use client首行全对；**源文件** 110个/12740行/34 API路由/16页面/25组件 |
| 2026-03-26 | V18.0 | M3 批次5全部完成✅（2项交付物）：**M3-17** api/auth/change-password/route.ts(64行-旧密码校验+bcrypt哈希+空密码检测+PRD合规密码规则min8+大小写+数字)；**M3-16** customer/profile/page.tsx(220行-用户信息展示+密码表单inline展开+👁可见性切换+即时校验+密码强度条+订单入口Link+退出登录二次确认+GlassCard动画)；**查漏补缺** 8项缺口发现(P0×2+P1×3+P2×3)：密码规则min(6)→min(8)+regex/空密码检测/MenuItem引用/退出重复/密码可见性切换/部门信息/密码强度条；**验收** tsc 0错误/build 0警告/test 74通过/as any 0/console.log 0/TODO 0/use client首行全对；**源文件** 115个/35 API路由/18页面；**M3进度** 5/8批次(62.5%) |
| 2026-03-25 | V17.0 | M3 批次4全部完成✅（4项缺口修复+3个交付物）：**深度分析** 8个缺口发现(P0×3+P1×3+P2×2)：NotificationItem缺orderId/Socket单例无生命周期/通知类型路由未定义/通知中心无分页/双重轮询浪费/通知图标映射重复/Socket断连无UI反馈/角标更新延迟；**新建3文件** notification-icons.ts(通知图标+路由映射共享常量)、use-socket-client.ts(72行-单例模式+Cookie认证+isConnected状态+disconnect方法+visibilitychange重连)、customer/notifications/page.tsx(185行-分页加载+未读/已读+类型图标+点击跳转订单+全部已读+加载更多)；**修改3文件** notification-store.ts(NotificationItem加orderId)、customer/layout.tsx(Socket实时刷新角标+isConnected控制轮询fallback)、notification-bell.tsx(复用NOTIFICATION_ICONS共享常量)；**验收** tsc 0错误/build 0警告/test 74通过/as any 0/console.log 0/TODO 0；**源文件** 113个/13069行/34 API路由/17页面/25组件；**M3进度** 4/8批次(50%) |
| 2026-03-26 | V19.0 | M3 批次6全部完成✅（3项子任务）：**M3-21** DocumentPanel集成文件删除(canDeleteFile权限+handleFileDelete+FilePreview onDelete prop-compact和卡片模式均支持)；**M3-22** 批量上传并发优化(串行for→3路并发池+按批更新进度条)；**M3-20** 4个API路由补充Socket实时推送(documents/[id] PATCH→DOC_REVIEWED客户、materials POST→MATERIAL_UPLOADED/MATERIAL_FEEDBACK资料员+客户、cancel POST→STATUS_CHANGE相关人员、reassign POST→ORDER_NEW目标用户)；**修改6文件** file-preview.tsx(+onDelete prop+删除按钮compact+card)、document-panel.tsx(+canDeleteFile+handleFileDelete+并发上传)、documents/[id]/route.ts(+emitToUser)、materials/route.ts(+emitToUser)、cancel/route.ts(+emitToUser)、reassign/route.ts(+emitToUser)；**通知链9节点全部闭合** DB+Socket双通道全部就绪；**验收** tsc 0错误/build 0警告/test 74通过/as any 0/console.log 0/TODO 0；**源文件** 115个/13460行/35 API路由/18页面/25组件；**M3进度** 6/8批次(75%) |
| 2026-03-26 | V20.0 | M3 批次7全部完成✅（全量验收+优化）：**查漏补缺**深度遍历全部115个源文件对照M3计划文档逐项核查，发现1项P1缺口（客户详情页缺少多人申请人只读展示）；**修复** customer/orders/[id]/page.tsx 新增申请人只读卡片（applicantCount>1时显示，含头像/姓名/护照/资料状态/出签结果+总进度统计）；**全局验收** tsc 0错误/build 0警告/test 74通过/as any 0/console.log 0/TODO 0/'use client'首行全部正确/内部导航全用Link/Prisma可选字段??null全部正确；**M3全链路验证** 9个通知节点DB+Socket双通道全部闭合+端到端15步流程逻辑验证通过；**文档更新** 全部6份文档版本号+日期更新；**源文件** 115个/13599行/35 API路由/18页面/25组件；**M3进度** 8/8批次(100%) ✅ |
| 2026-03-28 | V21.0 | **工作流完整性验证✅**：基于用户提供的完整业务工作流描述（5阶段20+步骤+管理端+客户端完整交互），逐句对比当前代码实现，三轮深度分析发现12项缺口（P0×3/P1×3/P2×4/SMS×2）；**P0修复** ①操作员审核资料时同步通知资料员(documents/[id]/PATCH增加通知+进度统计)②资料审核合格时也通知资料员(APPROVED含"3/5"进度)③资料员打回材料备注必填(StatusTransitionModal detailRequired+动态placeholder)；**P1修复** ④"提交复审"vs"提交审核"动态文案(orderLogs判断UNDER_REVIEW→COLLECTING_DOCS记录)⑤签证类型差异化结果反馈(电子签客户不可反馈/申根标签/其他可反馈)⑥客户端"全部合格"视觉提示(APPROVED时绿色完成标记)；**P2修复** ⑦Shop API预留(/api/shop/sync+/api/shop/webhook)⑧SMS预留(/api/sms/send+/api/sms/templates)⑨状态流转弹窗备注placeholder动态化；**验证** tsc 0错误/build 0错误/test 74通过；**源文件** 119个/13757行/39 API路由/18页面/25组件 |
| 2026-03-28 | V22.0 | **M4 全知手册V3.0深度审查修订**：逐文件比对全部119个源文件+数据库schema+现有API路由+中间件+Socket+OSS+RBAC+事件总线+种子数据，发现20项缺口（P0×6/P1×8/P2×4/建议×2）；**P0修复** ①presign API schema扩展(context/orderId参数，requirementId改可选)②confirm API chat模式不写DocumentFile返回ossUrl③oss.ts buildOssKey追加type='chat'④ChatRoom自动创建改到api/orders/route.ts POST(upsert防并发)⑤chat_system用户phone确认不冲突(13800000001 vs 13800000000)⑥useSocketClient单例回调覆盖→全局回调注册表方案；**P1修复** ⑦新增emitToRoom函数⑧系统消息触发覆盖10个工作流节点(transitionKey映射表)⑨迁移SQL枚举建表顺序明确⑩chat:join管理者权限校验⑪ChatRoom upsert防唯一约束异常⑫rooms API MySQL兼容(NULLS LAST→ORDER BY IS NULL)⑬消息分页复合游标(createdAt+id)⑭事务外Socket emit可接受；**文档更新** 07-M4-dev-plan→V3.0(20项缺口清单+14项架构决策+完整批次计划) |
| 2026-03-29 | V22.1 | **M4 批次 1 完成✅**（12 子任务）：**Schema** prisma/schema.prisma +ChatRoom/ChatMessage/ChatRead 3 Model +ChatRoomStatus/ChatMessageType 2 Enum +NotificationType 加 CHAT_MESSAGE +Company/User/Order 关联；**类型** 新建 src/types/chat.ts(ChatRoom/ChatMessageItem/SendMessagePayload/ChatRoomSummary+Socket事件payload) +types/order.ts NotificationType 修复(GAP-N1：+SYSTEM+CHAT_MESSAGE)；**认证** auth.ts JwtPayload +realName+avatar(GAP-N2) +login/register/refresh/reset-password 4路由 JWT payload 同步追加；**权限** rbac.ts Lv2-7,9 加 chat read/send，OUTSOURCE 不加；**种子** seed.ts +chat_system 用户(id=chat_system,phone=13800000001)；**订单创建** orders/route.ts POST ChatRoom upsert 移到事务最前面(GAP-N2 修复时序问题)；**系统消息** 新建 src/lib/chat-system.ts(sendSystemMessage+archiveChatRoom) +events.ts 11 节点 transitionKey 系统消息+终态归档；**Socket** socket.ts +emitToRoom 导出；**测试修复** rbac.test.ts JwtPayload 加 realName+avatar；**验收** tsc 0错误/build 0警告/74 tests pass/as any 0/console.log 0/TODO 0；**源文件** 119→121/~13757→14060 行；**文档更新** 07-M4-dev-plan→V6.0(新增5项缺口 GAP-N1~N5，累计37项) |
| 2026-03-29 | V22.1 | **M4 批次 2 完成✅**（5 API+5 修复）：**修复** types/order.ts NotificationType 重复 SYSTEM 删除；presign/route.ts +context('document'/'chat')+orderId 参数+requirementId 改可选+权限按 context 分支检查；confirm/route.ts +context/orderId+chat 不写 DocumentFile 返回 ossUrl+ossKey 校验按 context 分支；oss.ts buildOssKey type+'chat'；types/api.ts ApiMeta +hasMore/cursor 字段；**新建 API** GET /api/chat/rooms(会话列表+未读数原生SQL MySQL兼容)；GET /api/chat/rooms/[orderId](获取/创建会话upsert)；GET+POST /api/chat/rooms/[orderId]/messages(复合游标分页+发送消息事务+Socket推送)；POST /api/chat/rooms/[orderId]/read(标记已读upsert+Socket回执)；**验收** tsc 0错误/build 0警告/0警告/as any 0/console.log 0/TODO 0；**源文件** 121→126/~14060→14200 行/API 39→44 |
| 2026-03-29 | V25.0 | **M4 批次 5 完成✅ → M4 全部完成**：**深度审查**逐文件遍历全部M4相关代码(13个核心文件+5个组件+4个API+集成点)，发现10项缺口(P0×2/P1×5/P2×3)，全部修复；**P0修复** ①ChatRoomList路由按角色跳转(useAuth判断CUSTOMER→/customer/orders vs /admin/orders) ②移除console.warn→chatErrorHandlers分发；**P1修复** ③客户端浮动按钮加未读角标+挂载fetchRooms ④客户Tab角标含聊天未读(registerChatMessageHandler+fetchRooms) ⑤admin浮动按钮挂载fetchRooms ⑥useChat markRead去重(去掉冗余API) ⑦离线消息通知(messages POST→5min去重→CHAT_MESSAGE通知+Socket推送)；**P2修复** ⑧chat-message/chat-panel img→next/Image ⑨typingTimersRef cleanup修复 ⑩order.orderLogs→currentOrder.orderLogs(useCallback闭包)；**测试** chat-system.test.ts新增15用例；**验收** tsc 0错误/build 0警告0错误/test 89通过(74+15)/as any 0/console.log 0/TODO 0；**源文件** 132→133/~15200→~15300行 |
| 2026-03-29 | V24.0 | **M4 批次 3+4 完成✅**（Socket服务端+前端UI全链路） |
| 2026-03-30 | V26.0 | **M6 全部完成✅**：**初版**(commit 8d4fc6d)签证模板库CRUD(api/templates/3路由+admin/templates页面)、智能资料检查(api/orders/[id]/check-5项检查)、批量操作(api/orders/batch-3种操作)、团队管理(admin/team)、系统设置(admin/settings+api/companies/me)、订单管理批量选择栏；**补全**(commit 85f819e)签证分类流程差异化(VisaFlowHints组件-电子/贴纸/申根/指纹4类提示)、订单模板快速录入(api/order-templates+创建订单弹窗快速录入面板-模板填充+老客户自动填充)；**验收** tsc 0错误/build 0警告/test 93通过/as any 0/console.log 0/TODO 0；**源文件** 133→140/~15300→~18500行/44→45 API路由 |
| 2026-03-31 | V28.0 | **架构升级方案V3.0定稿**：**方案决策**否决V2.0拆分重构方案(需改101文件)→采纳门户化改造方案(仅改3文件+新增16文件)；**深度分析**逐文件扫描141个源文件实际依赖，发现events.ts耦合被高估(仅1个文件import)、prisma.ts(45引用)/auth.ts(47引用)才是核心依赖、rbac.ts有auth.ts type依赖(原文档遗漏)、server.ts/vitest.config.ts/tsconfig.json未纳入原方案分析；**文档更新** 08-architecture-redesign.md→V3.0(门户化改造完整方案，661行)、01-PRD→V17.0(+M7里程碑)、02-architecture→V17.0(+门户层路由)、03-project-status→V28.0(本条)、04-dev-standards→V16.0(+门户开发规范)；**ERP代码零改动** 141文件完全不动，零回归风险；**骨架工时** 3.5h(原方案17h节省80%) |
| 2026-03-31 | V29.0 | **架构升级方案V4.0定稿 — 分层模块化+门户化改造**：**方案升级** V3.0(不动存量纯增量)→V4.0(分层模块化架构)；用户需求"模块化代码架构、ERP作为其中一个功能板块、门户首页精心设计"驱动；**三层架构** shared/(纯公共基础设施)→modules/erp/(ERP业务模块隔离)→portal/(用户入口组装)；**模块化价值** 故障隔离(ERP bug不影响新模块)、可扩展(新建modules/xxx/即可)、import边界清晰(@shared/* @erp/* 路径别名)；**门户首页精心设计** Hero逐词渐显动画+底部光带脉动+数据卡片数字滚动(Intersection Observer)+6大工具stagger入场+hover sweep光效+热门目的地scroll-snap横向滚动+CTA按钮光泽扫过；**执行计划** Phase 0-5共~8h(目录重构3h+入口改造0.5h+门户首页3h+ERP入口0.5h+工具骨架1h)；**文件变更** 移动~50文件+新增16文件+修改~130文件(import路径)+tsconfig 2个新别名；**验证** 每阶段tsc 0错误+build通过+93 test pass=功能等价证明；**文档更新** 08-architecture-redesign.md→V4.0(970行分层模块化完整方案)、03-project-status→V29.0(源代码目录地图增加模块化预告+里程碑更新)；**源文件** 143个/~18649行/45 API路由/18页面/31组件/93测试用例 |

| 2026-04-01 | V30.0 | **架构合规深度审查+5项修复**：**审查范围** 161个源文件逐文件import扫描+模块边界验证+测试文件位置检查+迁移文件完整性+403页面角色兼容性；**P0修复** ①跨层引用违规：shared/hooks/use-socket-client.ts import @erp/types/chat(违反shared→modules边界)→新建shared/types/socket-events.ts，3个Socket payload类型提升到shared层，use-socket-client改引@shared/types/socket-events，modules/erp/types/chat.ts改为re-export ②M4迁移缺失：ChatRoom/ChatMessage/ChatRead Schema有但无migration→新建prisma/migrations/20260329_add_m4_chat_tables/migration.sql(3张表+全部索引)；**P1修复** ③测试文件错位：transition/desensitize/chat-system 3个ERP模块测试仍在shared/lib/__tests__/→移到modules/erp/lib/__tests__/ ④403页面硬编码：/admin/dashboard对客户不可访问→改'use client'+useAuth按角色分流(CUSTOMER→/，员工→/admin/dashboard) ⑤文档测试数量：多处写93 passed实际91→08-architecture-redesign+04-dev-standards统一修正；**架构边界确认** shared→@erp引用=0✅ / modules→@shared引用=58✅ / portal→@erp引用=0✅；**验收** tsc 0错误/build通过/91 tests pass；**源文件** 161个/~19900行/50 API路由/28页面/32组件/91测试用例；**文档更新** 08-architecture-redesign→V5.4(+Phase 4.5合规修复章节)、03-project-status→V30.0 |
| 2026-04-02 | V32.0 | **M7 Phase 5 工具模块内容开发完成✅**：**数据库** prisma/schema.prisma新增8个Model(NewsArticle+Itinerary+FormTemplate+FormRecord+VisaAssessment+TranslationRequest+DocHelperTemplate+GeneratedDocument)+迁移文件；**API** 新增7个路由(/api/news+/api/itineraries+/api/form-templates+/api/form-records+/api/visa-assessments+/api/translations+/api/doc-helper)；**页面** 6个portal工具页从占位替换为真实实现(资讯列表+行程CRUD+申请表填写+签证评估4步问卷+翻译工具+证明文件生成)；**全量审计** import路径/模块边界/tsc/build/tests全部通过；**初始迁移** 新增0000_init_full_schema完整Schema(381行)；**验收** tsc 0错误/build通过/91 tests pass；**源文件** 159→166个/19897→20770行/50→57 API路由/14→22 Prisma表；**文档更新** 08-architecture-redesign→V6.0(+Phase 5完成)、03-project-status→V31.0 |

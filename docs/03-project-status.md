...
> **文档版本**: V2.5  
> **更新日期**: 2026-03-20 18:30  
> **用途**: 本文件是项目唯一真相源。任何开发者/AI助手拿到本文件 + Git仓库即可完整恢复开发上下文。  
> **当前阶段**: M1 全部完成(12/12) ✅ → M2 核心工作流 进行中 (13/19)
...
| **代码统计** | 76 源文件 / ~7800 行 / 21 API 路由 / 14 页面 / 15 组件 |
...
| `documents/document-panel.tsx` | ✅ | 资料面板（需求列表+添加+上传+审核一体化） |
| `documents/material-panel.tsx` | ✅ | 签证材料面板（上传+版本管理+列表） |
...
| 4 | 状态机引擎 | ✅ | 12条规则 + 事务 + 日志 + 订单详情页动态按钮 |
| 5 | 文档级审核反馈系统 | ✅ | PATCH /api/documents/[id] + DocumentPanel 审核表单 |
| 6 | 资料收集面板组件 | ✅ | DocumentPanel（需求列表/添加/上传/审核一体化） |
| 7 | 通知系统 (Socket.io) | ✅ | 事件总线 + 创建订单通知 + 状态变更通知 |
| 8 | 签证材料管理 | ✅ | Materials API + MaterialPanel（上传/版本/列表） |
| 9 | 客服工作台 UI | ✅ | workspace 页面（快捷统计 + 订单列表） |
| 10 | 资料员工作台 UI | ✅ | 公共池页面 + workspace 页面 |
| 11 | 操作员工作台 UI | ✅ | 公共池页面 + workspace 页面 |
| 12 | 订单详情页（多角色） | ✅ | 信息展示 + 动态状态按钮 + 资料面板 + 材料面板 |
| 13 | 预约站内消息提醒 | ⬜ | Cron 定时任务（阶段五） |
| 14 | 操作日志系统 | ✅ | 全流程写入（创建/接单/状态/审核/上传） |
| 15 | 文字消息备注 | ✅ | 审核时填写 rejectReason + 状态流转 detail 字段 |
| 16 | 文件处理增强 | ⚠️ | 已支持 TXT+多文件上传；HEIC转换/上传进度待做 |
| 17 | 手机拍照上传+取景框 | ⬜ | getUserMedia + Canvas（阶段五） |
| 18 | 文件预览系统 | ⬜ | PDF/图片/TXT内嵌预览（阶段三） |
| 19 | 异常与边界处理 | ⬜ | 取消/超时/转单/重办（阶段五） |

...
> **文档版本**: V4.0  
> **更新日期**: 2026-03-21 02:41  
> **用途**: 本文件是项目唯一真相源。任何开发者/AI助手拿到本文件 + Git仓库即可完整恢复开发上下文。  
> **当前阶段**: M1 全部完成(12/12) ✅ → M2 核心工作流 全部完成(19/19) ✅ → M3 待开发  
...
| **代码统计** | 84 源文件 / ~9225 行 / 25 API 路由 / 14 页面 / 17 组件 |
...
### M5 多申请人 + 数据看板 — 任务清单（14 项）

> 基于公司现有手工 Excel 统计表分析，实现多申请人支持 + 财务统计 + 数据看板 + Excel 导出。

#### M5-Phase A：多申请人（5 项）

| # | 任务 | 说明 | 工作量 |
|---|---|---|:---:|
| M5-1 | Applicant 数据模型 | 新建 `erp_applicants` 表 + Order 扩展字段（contactName/targetCity/submittedAt/财务字段） | 0.5天 |
| M5-2 | 创建订单表单（多人支持） | 客服录单时动态添加/删除申请人，至少1人 | 1天 |
| M5-3 | 资料收集面板（按人分组） | 每个 Applicant 独立标记 documentsComplete，全部齐全才能提交审核 | 0.5天 |
| M5-4 | 订单详情（申请人卡片） | 展示申请人列表，独立资料状态+出签结果 | 0.5天 |
| M5-5 | 状态流转（多人结果判断） | 全部出签→APPROVED / 全部拒签→REJECTED / 混合→PARTIAL | 0.5天 |

#### M5-Phase B：数据看板（5 项）

| # | 任务 | 说明 | 工作量 |
|---|---|---|:---:|
| M5-6 | 分析 API（概览/趋势/绩效） | GET /api/analytics/overview + /trend + /workload | 1天 |
| M5-7 | 仪表盘图表组件 | 月度趋势折线图、国家分布饼图、出签率仪表盘、支付方式分布 | 1天 |
| M5-8 | 客服/操作员绩效排行榜 | 按人员统计订单数、金额、出签率 | 0.5天 |
| M5-9 | 筛选器 | 月份范围、国家、客服、操作员多维度筛选 | 0.5天 |
| M5-10 | 异常订单预警列表 | 超时未推进订单实时展示 | 0.5天 |

#### M5-Phase C：导出与迁移（4 项）

| # | 任务 | 说明 | 工作量 |
|---|---|---|:---:|
| M5-11 | Excel 导出 API | GET /api/analytics/export?month=2026-03&format=xlsx，列结构对齐手工表 | 1天 |
| M5-12 | 多人订单导出行合并 | 同手工表格式：联系人行合并，多人在"申请人"列分行展示 | 0.5天 |
| M5-13 | 历史数据批量导入脚本 | 解析15个月Excel → 创建 Order + Applicant + 财务数据 | 1天 |
| M5-14 | 财务自动计算 | 毛利 = 金额 - 平台费 - 签证费 - 保险 - 好评返现，触发器自动算 | 0.5天 |

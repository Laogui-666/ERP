...
| **代码统计** | 57 源文件 / ~4700 行 / 13 API 路由 / 5 页面 / 13 组件 |
...
# 7. 启动开发服务器
npm run dev
# 或（需要 Socket.io 时，待 custom server.ts 实现后）
# npx tsx server.ts
...
| `orders/[id]/claim/route.ts` | POST | `/api/orders/[id]/claim` | ⚠️ | 接单（有竞争条件，待修复#2） |
...
| `events.ts` | ⚠️ | 事件总线已定义但未被任何代码调用（死代码，待修复#6） |
| `socket.ts` | ⚠️ | Socket.io 服务端已定义但未启动（待修复#5） |
| `oss.ts` | ⚠️ | OSS 占位实现，返回假 URL（待修复#7） |
...
| 文件 | 状态 | 说明 |
|---|:---:|---|
| `middleware.ts` | ⚠️ | 公开路由/鉴权/权限/角色跳转（客户跳转顺序待修复#12） |
...
| 订单 CRUD | ✅ | 创建/列表/详情/更新 |
| 状态机 | ✅ | 12条规则 + 事务 + 日志 |
| 公共池 + 接单 | ⚠️ | 逻辑完整但有并发问题#2 |
| 客户自动创建 | ✅ | 创建订单时自动注册客户（首次登录待修复#4） |
| 资料管理 | 🔲 | M2 开发 |
| 签证材料管理 | 🔲 | M2 开发 |
| 通知系统 | ⚠️ | 数据模型完整，事件总线/Socket.io 未集成 |
| 文件上传 | ⚠️ | 占位实现，未接入 OSS |
| 数据看板 | 🔲 | M5 开发 |
| 客户端门户 | ⚠️ | 基础框架 |
| SMS | 🔲 | 501 预留 |
...
| 7 | OSS 接入阿里云 SDK | ⬜ | — | ⬜ 第三批 |
| 8 | HX订单号 + 外部订单号 | ✅ | ✅ | ✅ 已完成 |
| 9 | NotificationType enum | ✅ | ✅ | ✅ 已完成 |
| 10 | OUTSOURCE 脱敏调用 | ⬜ | — | ⬜ 第四批 |
| 11 | DocumentFile sortOrder+label | ✅ | ✅ | ✅ 已完成 |
| 12 | 中间件客户跳转顺序 | ⬜ | — | ⬜ 第四批 |
...
| 风险 | 等级 | 应对方案 |
|---|:---:|---|
| 接单并发竞争 | 高 | 修复#2：乐观锁 WHERE |
| 客户无法登录 | 高 | 修复#4：首次登录改密流程 |
| Socket.io 未启动 | 高 | 修复#5：custom server.ts |
| 事件总线死代码 | 高 | 修复#6：集成到 transition |
| OSS 占位实现 | 高 | 修复#7：接入阿里云 SDK |
| OUTSOURCE 脱敏未应用 | 中 | 修复#10：GET 列表接口调用 |
| 中间件顺序错误 | 中 | 修复#12：调整判断顺序 |
| Next.js 14.2.18 安全漏洞 | 中 | 待升级 |
| backdrop-blur 性能 | 中 | CSS 降级方案 |
| 大文件上传超时 | 中 | OSS 预签名直传 |
| 文件安全 | 中 | magic bytes 校验 |
| 订单长时间无进展 | 中 | Cron 超时检测 |

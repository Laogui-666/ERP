...
> **文档版本**: V5.2
> **更新日期**: 2026-03-22 02:00
> **用途**: 本文件是项目唯一真相源。任何开发者/AI助手拿到本文件 + Git仓库即可完整恢复开发上下文。
> **当前阶段**: M1 ✅ → M2 ✅ → **M5 开发中（批次 1-5 完成，批次 6-8 待开发）**
...
| **代码统计** | 82 源文件 / ~9133 行 / 26 API 路由 / 15 页面 / 19 组件 |
...
M3 ████████████████████████████████████████ ░░░░  0%      Week 6-8
M4 ████████████████████████████████████████ ░░░░  0%      Week 9-10
M5 ████████████████████████████████████████ ▓▓▓▓▓▓░░  62%     Week 11-13 (批次1-5完成, 批次6-8待开发)
M6 ████████████████████████████████████████ ░░░░  0%      Week 14-15
```
...
| 2026-03-22 | V5.2 | M5批次3-5完成+全面深度审查修复：**批次3** GET/POST/PATCH orders API扩展(applicants关联+多申请人创建+财务字段+自动重算毛利)；**批次4** PATCH/GET /api/applicants/[id]（含autoResolveOrderStatus+终态通知）；**批次5** ApplicantCard组件(出签/拒签/资料状态)、ApplicantFormItem组件(动态增删)、订单详情页申请人区域+PARTIAL action、创建订单表单多申请人+财务+毛利预览；**审查修复** document-panel orderStatus约束(将_orderStatus改为实际使用+isEditableStatus)、客户端订单进度指示器PARTIAL支持、全面工作流5阶段对齐验证、权限矩阵9级验证、26个API路由完整性验证；源文件79→82/8425→9133行/API 25→26/组件17→19；tsc 0错误/build通过/as any 0残留；五份文档全部更新至最新版本 |

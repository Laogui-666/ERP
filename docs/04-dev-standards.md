...
> **核心原则：对本项目的全部增删改、修复或调整，禁止临时性的打补丁补救。必须从根本上解决问题，确保修复后系统行为正确且无副作用。**

### 1.5.1 什么是"打补丁"

| ❌ 打补丁（禁止） | ✅ 根治（正确） |
|---|---|
| 用 `if` 特判绕过问题 | 找到根本原因并修复 |
| 在调用处转换类型 (`as unknown as`) | 修复类型定义使其匹配 |
| 硬编码值替代动态获取 | 修复数据源或计算逻辑 |
| 忽略 TS 错误用 `@ts-ignore` | 修复类型使其正确 |
| 表面修改 UI 遮盖 bug | 修复后端逻辑保证数据正确 |
| 在组件内写 `TODO: 待优化` | 直接实现或移除 |

### 1.5.2 修复流程

```
发现问题 → 定位根因 → 设计方案 → 评估副作用 → 执行修复 → 全量验证(tsc+build) → 提交
```

**每一步都必须执行：**

1. **定位根因**：不要只看报错表面，要追溯到数据/逻辑/类型的源头
2. **设计方案**：修复方案必须保证：
   - 不引入新的 bug
   - 不破坏现有功能
   - 类型安全（`tsc --noEmit` 0 错误）
   - 构建通过（`npm run build`）
3. **评估副作用**：修改影响范围内的所有代码都要检查
4. **全量验证**：提交前必须运行 `npx tsc --noEmit && npm run build`

### 1.5.3 特别禁止项

- ❌ 禁止在 Prisma 查询结果上用 `as any` 绕过类型检查
- ❌ 禁止在状态流转中用 `as unknown as OrderStatus` 强转
- ❌ 禁止添加 `_unusedParam` 以消除 TS 未使用变量警告（应直接删除或使用）
- ❌ 禁止用 `// eslint-disable` 压制 lint 警告而不修复根本原因
- ❌ 禁止在前端用 `try/catch` 静默吞掉错误而不向用户反馈

### 1.5.4 Code Review 检查（新增）

在 Code Review 清单中新增检查项：

- [ ] 修复方案是否从根本解决问题，而非绕过
- [ ] 是否有 `as any` / `as never` / `as unknown as` 转型
- [ ] 是否有未使用的参数（`_param` 前缀）
- [ ] 修改后 `tsc --noEmit` 是否 0 错误
- [ ] 修改后 `npm run build` 是否通过
- [ ] 修改是否影响其他模块的功能
...
| 层级 | 工具 | 覆盖目标 | 要求 |
|---|---|---|---|
| 单元测试 | Vitest | 工具函数、Service 层 | 核心逻辑 100% |
| 集成测试 | Vitest + Prisma | API Routes | 关键流程 100% |
| E2E 测试 | Playwright | 用户操作流程 | 核心工作流 |

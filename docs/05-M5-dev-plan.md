...
批次 3 ✅ — 订单 API 扩展（2h）
  ...
  └── PATCH /api/orders/[id]     +财务字段白名单 +自动重算grossProfit
  验收: npx tsc --noEmit = 0 错误 ✅

批次 4 ✅ — 申请人 API（1.5h）
  ├── PATCH /api/applicants/[id]  含autoResolveOrderStatus调用+通知
  └── GET  /api/applicants/[id]   申请人详情查询
  验收: npx tsc --noEmit = 0 错误 ✅

批次 5 ✅ — 前端多申请人（3h）
  ├── src/components/orders/applicant-card.tsx     新组件（出签/拒签/资料状态）
  ├── src/components/orders/applicant-form-item.tsx 新组件（动态增删）
  ├── src/app/admin/orders/page.tsx                创建表单重写（多申请人+财务+毛利预览）
...
**已完成：批次 1-5（~10.5h） | 剩余：批次 6-8（~8.5h） | 总计：~19 小时（2.5 天）**

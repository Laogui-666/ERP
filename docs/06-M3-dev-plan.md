# 华夏签证 ERP - M3 全知开发手册（V9.0 终版）

> **文档版本**: V14.0
> **更新日期**: 2026-03-30 02:30
> **用途**: M3 阶段唯一开发指南。即使丢失所有上下文，拿到本文件 + Git 仓库即可完整恢复开发。
> **前置条件**: M1 ✅ + M2 ✅ + M5 ✅ 全部完成（119 源文件 / ~13,757 行 / 39 API 路由 / 18 页面 / 25 组件 / 74 测试用例）
...
> **当前阶段**: M3 全部 8 批次完成 ✅ | 下一步：M4 实时通信 / M6 行业优化
...
| M3 文件与客户端 | ✅ 100%（全部 8 批次完成，25h 计划） |
...
批次 7（2h）— 全量验收 ✅ 已完成 2026-03-26
  ├── tsc 0错误 / build 0警告 / test 74通过 ✅
  ├── 客户详情页申请人只读卡片 ✅
  └── 文档版本号更新 + Git commit + push ✅
```
...
### 批次 7（最终验收）✅ 全部通过
- [x] 全部 9 项全局检查通过（tsc 0错误/build 0警告/test 74通过/as any 0/console.log 0/TODO 0/'use client'首行/Link导航/Prisma ??null）
- [x] 端到端 15 步流程逻辑验证通过
- [x] 客户详情页申请人只读卡片（batch 7 新增优化）
- [x] Git commit + push

---

## 附录：工作流完整性验证（2026-03-28）

M3 全部完成后，基于用户提供的完整业务工作流描述（5阶段20+步骤）进行了第四轮深度对比分析。

### 发现的缺口与修复

| # | 级别 | 阶段 | 缺口 | 修复 | 文件 |
|---|:---:|:---:|---|---|---|
| 1 | 🔴P0 | 三-2 | 操作员审核资料不通知资料员 | PATCH 增加通知资料员+进度统计 | documents/[id]/route.ts |
| 2 | 🔴P0 | 三-2 | APPROVED 时资料员无感知 | APPROVED 也通知（含"3/5"进度） | documents/[id]/route.ts |
| 3 | 🔴P0 | 四-2 | 资料员打回材料缺备注 | PENDING_DELIVERY→MAKING_MATERIALS 备注必填 | admin/orders/[id]/page.tsx |
| 4 | 🟡P1 | 三-4 | "提交复审" vs "提交审核" | orderLogs 判断是否被打回过 | admin/orders/[id]/page.tsx |
| 5 | 🟡P1 | 五 | 签证类型不区分反馈方式 | 电子签不可反馈/申根标记 | customer/orders/[id]/page.tsx |
| 6 | 🟡P1 | 二-5 | 客户端缺"全部合格"提示 | 全部 APPROVED 绿色标记 | customer/orders/[id]/page.tsx |
| 7 | 🟢P2 | 一 | Shop API 未预留 | /api/shop/sync + /api/shop/webhook | 新建 2 文件 |
| 8 | 🟢P2 | 全 | SMS 接口未创建 | /api/sms/send + /api/sms/templates | 新建 2 文件 |
| 9 | 🟢P2 | 全 | 状态流转 placeholder 不明确 | 根据操作动态生成 | admin/orders/[id]/page.tsx |

### 通知链完整度（修复后）

| # | 触发时机 | 接收者 | 类型 | 通知内容 | 状态 |
|---|---|---|---|---|:---:|
| N1 | 客服创建订单 | 资料员 | ORDER_NEW | "又有新订单啦" | ✅ |
| N2 | 客服创建订单 | 客户 | ORDER_CREATED | "订单已创建，资料员将在24h内联系" | ✅ |
| N3 | 状态变更（任何） | 相关人员 | STATUS_CHANGE | "订单XXX 状态变更" | ✅ |
| N4 | 资料员发送清单/追加 | 客户 | DOC_REVIEWED | "资料清单已更新，请及时上传" | ✅ |
| N5 | 客户确认提交 | 资料员 | DOCS_SUBMITTED | "客户已提交N项资料" | ✅ |
| N6 | 资料审核驳回 | 客户 | DOC_REVIEWED | "资料X需要修改：原因" | ✅ |
| **N6b** | **资料审核驳回** | **资料员** | **DOC_REVIEWED** | **"资料X需修改：原因"** | **✅ 本次新增** |
| **N6c** | **资料审核合格** | **资料员** | **DOC_REVIEWED** | **"资料X已合格（3/5）"** | **✅ 本次新增** |
| N7 | 材料上传 | 资料员+客户 | MATERIAL_UPLOADED | "签证材料已上传/更新" | ✅ |
| N8 | 取消订单 | 相关人员 | STATUS_CHANGE | "订单已取消" | ✅ |
| N9 | 转单 | 目标用户 | ORDER_NEW | "有新订单转入" | ✅ |

---

*文档结束 — M3 全知手册 V13.0 终版（全部8批次完成 + 工作流完整性验证）*

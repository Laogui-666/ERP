  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-4 bg-gradient-to-br from-morandi-light via-morandi-cream to-morandi-blush">
      <div className="relative z-10 w-full max-w-[400px]">
        ...
        >
          <motion.div 
            className="mx-auto mb-5 flex h-[68px] w-[68px] items-center justify-center rounded-2xl bg-morandi-ocean text-white shadow-lg"
            initial={{ scale: 0.8, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            transition={{ ...liquidSpringConfig.bouncy, delay: 0.2 }}
          >
          ...
          <motion.h1 
            className="text-3xl font-bold text-morandi-deep tracking-tight"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          <motion.p 
            className="mt-2 text-morandi-mist text-sm tracking-wide"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            ...
            <div className="space-y-6">
              <form onSubmit={handleLogin} className="space-y-5">
                <LiquidInput
                  ...
                  required
                  leftIcon={
                    <svg className="h-5 w-5 text-morandi-mist/60" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              ...
              <div className="mt-6 text-center">
                <span className="text-sm text-morandi-mist">
                  还没有账号？
                  <Link href="/register" className="ml-1.5 text-morandi-ocean hover:text-morandi-ocean/80 transition-colors duration-200 font-medium">
                    注册入驻
                  </Link>
                </span>
              </div>
        ...
        {/* 底部 */}
        <motion.p 
          className="mt-8 text-center text-xs text-morandi-mist/60 tracking-wide"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}

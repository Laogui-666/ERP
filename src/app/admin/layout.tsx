...
export default function AdminLayout({
  ...
  return (
    <div className="min-h-screen glass-background">
      ...
      {/* 移动端顶栏 */}
      <header className="fixed top-0 left-0 right-0 h-14 flex items-center px-4 z-30 md:hidden glass-navbar glass-navbar-animate">
        <button
          onClick={() => setSidebarOpen(true)}
          className="p-2 -ml-2 rounded-glass-sm text-glass-text-muted hover:text-glass-text-primary hover:bg-glass-bg-card active:scale-90 transition-all glass-button-hover"
        >
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 6h16M4 12h16M4 18h16" />
          </svg>
        </button>
        <span className="ml-3 text-sm font-bold text-glass-text-primary tracking-tight glass-text-gradient">Visa ERP</span>
      </header>

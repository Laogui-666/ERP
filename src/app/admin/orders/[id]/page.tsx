...
import type { OrderStatus, VisaResult } from '@/types/order'
...
export default function OrderDetailPage() {
  ...
  // M5：申请人出签/拒签
  const handleApplicantResultChange = async (applicantId: string, result: VisaResult, note?: string) => {
    const res = await fetch(`/api/applicants/${applicantId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ visaResult: result, visaResultNote: note }),
    })
    const json = await res.json()
    if (json.success) {
      toast('success', result === 'APPROVED' ? '已标记出签' : '已标记拒签')
      fetchOrder(orderId)
    } else {
  ...
  // M5：申请人资料状态切换
  const handleApplicantDocumentsChange = async (applicantId: string, complete: boolean) => {
    const res = await fetch(`/api/applicants/${applicantId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
    ...
    const json = await res.json()
    if (json.success) {
      toast('success', complete ? '已标为齐全' : '已标为待补充')
      fetchOrder(orderId)
    } else {
      toast('error', json.error?.message ?? '操作失败')
    }
              ...
              <div className="space-y-2">
                {order.applicants.map((applicant) => (
                  <ApplicantCard
                    key={applicant.id}
                    applicant={applicant}
                    canMarkResult={order.status === 'DELIVERED' && ['OPERATOR', 'DOC_COLLECTOR', 'VISA_ADMIN'].includes(user?.role ?? '')}
                    onResultChange={handleApplicantResultChange}
                    onDocumentsChange={handleApplicantDocumentsChange}
                  />
                ))}
              </div>
            </GlassCard>

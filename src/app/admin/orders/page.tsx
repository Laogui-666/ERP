...
import { ApplicantFormItem, type ApplicantFormItemData } from '@/components/orders/applicant-form-item'
...
function CreateOrderModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  // M5：多申请人
  const [applicants, setApplicants] = useState<ApplicantFormItemData[]>([
  ...
  const handleFinanceChange = (field: string, value: string) => {
    setFinance((prev) => ({ ...prev, [field]: value }))
  }

  // M5：申请人操作
  const handleApplicantChange = (index: number, field: keyof ApplicantFormItemData, value: string) => {
    setApplicants((prev) => prev.map((a, i) => (i === index ? { ...a, [field]: value } : a)))
  }

  const addApplicant = () => {
    setApplicants((prev) => [...prev, { name: '', phone: '', passportNo: '' }])
  }
  ...
  const previewPlatformFee = calcPlatformFee(amount, finance.platformFeeRate)
  const previewGrossProfit = calcGrossProfit({
    amount,
    platformFeeRate: finance.platformFeeRate,
    visaFee: finance.visaFee ? parseFloat(finance.visaFee) : null,
    insuranceFee: finance.insuranceFee ? parseFloat(finance.insuranceFee) : null,
    rejectionInsurance: finance.rejectionInsurance ? parseFloat(finance.rejectionInsurance) : null,
    reviewBonus: finance.reviewBonus ? parseFloat(finance.reviewBonus) : null,
  })
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload: Record<string, unknown> = {
        ...
        platformFeeRate: finance.platformFeeRate,
        visaFee: finance.visaFee ? parseFloat(finance.visaFee) : undefined,
        insuranceFee: finance.insuranceFee ? parseFloat(finance.insuranceFee) : undefined,
        rejectionInsurance: finance.rejectionInsurance ? parseFloat(finance.rejectionInsurance) : undefined,
        reviewBonus: finance.reviewBonus ? parseFloat(finance.reviewBonus) : undefined,
      }

      // 多人时传 applicants，1人时不传（向后兼容）
      if (validApplicants.length > 1 || validApplicants[0].name !== form.customerName) {
        payload.applicants = validApplicants.map((a) => ({
          name: a.name,
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input className={inputClass} placeholder="联系人姓名" value={form.customerName}
                onChange={(e) => handleChange('customerName', e.target.value)} />
            </div>
            <div>
              <label className={labelClass}>手机号 *</label>
              <input className={inputClass} placeholder="138xxxxxxxx" value={form.customerPhone}
                onChange={(e) => handleChange('customerPhone', e.target.value)} maxLength={11} />
            </div>
            <div>
              <label className={labelClass}>邮箱</label>
              <input className={inputClass} placeholder="example@mail.com" type="email"
                value={form.customerEmail} onChange={(e) => handleChange('customerEmail', e.target.value)} />
            </div>
            <div>
              <label className={labelClass}>护照号</label>
              <input className={inputClass} placeholder="E12345678" value={form.passportNo}
        ...
        <div>
          <div className="flex items-center justify-between mb-3 pb-2 border-b border-white/5">
            <h4 className="text-sm font-medium text-[var(--color-text-primary)]">
              申请人（{applicants.length}人）
            </h4>
            <button type="button" onClick={addApplicant}
              className="text-xs px-3 py-1.5 rounded-lg bg-[var(--color-primary)]/10 text-[var(--color-primary-light)] border border-[var(--color-primary)]/20 hover:bg-[var(--color-primary)]/20 transition-all">
              + 添加申请人
            </button>
          </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input className={inputClass} placeholder="如：法国" value={form.targetCountry}
                onChange={(e) => handleChange('targetCountry', e.target.value)} />
            </div>
            <div>
              <label className={labelClass}>签证类型 *</label>
              <input className={inputClass} placeholder="如：旅游" value={form.visaType}
                onChange={(e) => handleChange('visaType', e.target.value)} />
            </div>
            <div>
              <label className={labelClass}>签证类别</label>
              <input className={inputClass} placeholder="如：贴纸签" value={form.visaCategory}
                onChange={(e) => handleChange('visaCategory', e.target.value)} />
            </div>
            <div>
              <label className={labelClass}>送签城市</label>
              <input className={inputClass} placeholder="如：北京" value={form.targetCity}
                onChange={(e) => handleChange('targetCity', e.target.value)} />
            </div>
            <div>
              <label className={labelClass}>出行日期</label>
              <input className={inputClass} type="date" value={form.travelDate}
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input className={inputClass} type="number" placeholder="0.00" min="0" step="0.01"
            ...
            <div>
              ...
              <input className={inputClass} placeholder="如：淘宝、门店" value={form.sourceChannel}
                onChange={(e) => handleChange('sourceChannel', e.target.value)} />
            </div>
            <div>
              <label className={labelClass}>外部订单号</label>
              <input className={inputClass} placeholder="网店/第三方订单号" value={form.externalOrderNo}
                onChange={(e) => handleChange('externalOrderNo', e.target.value)} />
            </div>
          </div>
          <div className="mt-3">
            <label className={labelClass}>备注</label>
            <textarea className={inputClass + ' resize-none'} rows={2} placeholder="订单备注信息"
              value={form.remark} onChange={(e) => handleChange('remark', e.target.value)} />
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select className={inputClass}
                value={finance.platformFeeRate}
                onChange={(e) => handleFinanceChange('platformFeeRate', e.target.value)}>
            ...
            <div>
              ...
              <input className={inputClass} type="number" placeholder="0" min="0" step="0.01"
                value={finance.visaFee} onChange={(e) => handleFinanceChange('visaFee', e.target.value)} />
            </div>
            <div>
              <label className={labelClass}>申根保险 (¥)</label>
              <input className={inputClass} type="number" placeholder="0" min="0" step="0.01"
                value={finance.insuranceFee} onChange={(e) => handleFinanceChange('insuranceFee', e.target.value)} />
            </div>
            <div>
              <label className={labelClass}>拒签保险 (¥)</label>
              <input className={inputClass} type="number" placeholder="0" min="0" step="0.01"
                value={finance.rejectionInsurance} onChange={(e) => handleFinanceChange('rejectionInsurance', e.target.value)} />
            </div>
            <div>
              <label className={labelClass}>好评返现 (¥)</label>
              <input className={inputClass} type="number" placeholder="0" min="0" step="0.01"
                value={finance.reviewBonus} onChange={(e) => handleFinanceChange('reviewBonus', e.target.value)} />
            </div>
          </div>
          {/* 预览 */}
          {amount > 0 && (
            <div className="mt-3 p-3 rounded-xl bg-white/[0.03] border border-white/5 text-xs space-y-1">
              <div className="flex justify-between">
                <span className="text-[var(--color-text-placeholder)]">平台费用</span>
                <span className="text-[var(--color-text-secondary)]">¥{previewPlatformFee.toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[var(--color-text-placeholder)]">预估毛利</span>
                <span className={previewGrossProfit >= 0 ? 'text-[var(--color-success)]' : 'text-[var(--color-error)]'}>

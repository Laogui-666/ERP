...
export async function PATCH(
  ...
  try {
    ...
    if (data.visaFee !== undefined) updateData.visaFee = data.visaFee
    if (data.insuranceFee !== undefined) updateData.insuranceFee = data.insuranceFee
    if (data.rejectionInsurance !== undefined) updateData.rejectionInsurance = data.rejectionInsurance
    if (data.reviewBonus !== undefined) updateData.reviewBonus = data.reviewBonus
    // 自动重算毛利
    if (data.amount !== undefined || data.platformFeeRate !== undefined || data.visaFee !== undefined
      || data.insuranceFee !== undefined || data.rejectionInsurance !== undefined || data.reviewBonus !== undefined) {
      const { calcGrossProfit } = await import('@/lib/utils')
      const amount = data.amount ?? Number(existing.amount)
      const platformFeeRate = data.platformFeeRate ?? (existing.platformFeeRate ? Number(existing.platformFeeRate) : null)
      const visaFee = data.visaFee ?? (existing.visaFee ? Number(existing.visaFee) : null)
      const insuranceFee = data.insuranceFee ?? (existing.insuranceFee ? Number(existing.insuranceFee) : null)
      const rejectionInsurance = data.rejectionInsurance ?? (existing.rejectionInsurance ? Number(existing.rejectionInsurance) : null)
      const reviewBonus = data.reviewBonus ?? (existing.reviewBonus ? Number(existing.reviewBonus) : null)
      updateData.grossProfit = calcGrossProfit({ amount, platformFeeRate, visaFee, insuranceFee, rejectionInsurance, reviewBonus })
    }

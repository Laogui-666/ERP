    ...
    return (
      <div className="space-y-4 animate-pulse">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2"><div className="w-5 h-5 rounded bg-white/10" /><div className="w-32 h-5 rounded bg-white/10" /></div>
          <div className="w-16 h-5 rounded-full bg-white/10" />
        </div>
        <GlassCard className="p-5 space-y-4">
          {[1,2,3].map(i => <div key={i} className="flex gap-3"><div className="w-7 h-7 rounded-full bg-white/10 shrink-0" /><div className="space-y-1 flex-1"><div className="w-20 h-4 rounded bg-white/10" /><div className="w-16 h-3 rounded bg-white/5" /></div></div>)}
        </GlassCard>
      </div>
    )
  }
  ...
  if (!order) {
    return (
      <div className="p-12 text-center">
        ...
        <button onClick={() => router.push('/customer/orders')} className="mt-4 text-sm text-[var(--color-info)] hover:underline">返回订单列表</button>
      </div>
    )
  }
    ...
    <div className="space-y-5 pb-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button onClick={() => router.push('/customer/orders')} className="text-[var(--color-text-placeholder)] hover:text-[var(--color-text-secondary)] transition-colors">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /></svg>
          </button>
          <span className="font-mono text-sm text-[var(--color-primary-light)]">{order.orderNo}</span>
        </div>
      ...
      <div className="text-sm text-[var(--color-text-secondary)] flex items-center gap-2">
        ...
        <span className="text-[var(--color-text-placeholder)]">·</span><span>{order.visaType}</span>
        {order.applicantCount > 1 && (
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-[var(--color-accent)]/15 text-[var(--color-accent)]">👥 {order.applicantCount}人</span>
        )}
      </div>
      ...
      {/* ===== 多申请人标签页 ===== */}
      {showMultiTabs && (
        <GlassCard className="p-0 overflow-hidden animate-fade-in-up" style={{ animationDelay: '50ms' }}>
          {/* Tab 标签栏 */}
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-4 py-3 text-xs font-medium whitespace-nowrap transition-all border-b-2 ${
                    isActive
                      ? 'border-[var(--color-primary)] text-[var(--color-text-primary)] bg-white/[0.03]'
                      : 'border-transparent text-[var(--color-text-placeholder)] hover:text-[var(--color-text-secondary)] hover:bg-white/[0.02]'
                  }`}
                >
                  <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-medium shrink-0 ${
                    isActive ? 'bg-[var(--color-primary)]/20 text-[var(--color-primary)]' : 'bg-white/[0.06] text-[var(--color-text-placeholder)]'
                  }`}>{a.name[0]}</span>
                  <span>{a.name}</span>
                  {a.documentsComplete && <span className="text-[var(--color-success)]">✓</span>}
      ...
      {/* ===== B类材料 ===== */}
      {showMaterials && (
        <><div className="h-px bg-white/[0.06]" /><MaterialChecklist status={order.status} materials={order.visaMaterials} /></>
      )}
          ...
          <div className="space-y-3">
            ...
            {isEVisa ? (
              <div className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-4 text-center">
                <p className="text-xs text-[var(--color-text-secondary)]">电子签证结果将由送签专员反馈，届时会通知您</p>
              </div>
            ) : showFeedback ? (
              <div className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-4 space-y-3">
                <p className="text-xs text-[var(--color-text-secondary)]">请确认您的签证结果：</p>
                <div className="flex gap-3">
                  <button onClick={() => handleFeedback('APPROVED')} className="flex-1 py-2.5 rounded-xl bg-[var(--color-success)]/15 text-[var(--color-success)] text-sm font-medium hover:bg-[var(--color-success)]/25 transition-colors">✅ 已出签</button>
                  <button onClick={() => handleFeedback('REJECTED')} className="flex-1 py-2.5 rounded-xl bg-[var(--color-error)]/15 text-[var(--color-error)] text-sm font-medium hover:bg-[var(--color-error)]/25 transition-colors">❌ 被拒签</button>
                </div>
                <button onClick={() => setShowFeedback(false)} className="w-full text-xs text-[var(--color-text-placeholder)] hover:text-[var(--color-text-secondary)] transition-colors">取消</button>
              </div>
            ) : (
              <button onClick={() => setShowFeedback(true)} className="w-full py-3 rounded-xl border border-[var(--color-primary)]/30 text-[var(--color-primary)] text-sm font-medium hover:bg-[var(--color-primary)]/10 transition-colors">确认签证结果</button>
            )}
          </div>
        </>
      )}
      ...
      <GlassCard className="p-5 space-y-1">
        <h3 className="text-sm font-semibold text-[var(--color-text-primary)] flex items-center gap-2 mb-3"><span>📋</span><span>订单信息</span></h3>
  ...
  return (
    <>
      ...
      >
        ...
        {unread > 0 && !isOpen && (
          <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] rounded-full bg-[var(--color-error)] text-[10px] text-white flex items-center justify-center px-1 font-medium">{unread > 9 ? '9+' : unread}</span>
        )}
      </button>
      {isOpen && (
        <div className="fixed z-50 max-sm:inset-0 max-sm:rounded-none sm:bottom-20 sm:right-4 sm:w-96 sm:h-[500px] sm:rounded-2xl overflow-hidden shadow-2xl border border-white/[0.08] animate-fade-in-up" style={{ animationDuration: '200ms' }}>

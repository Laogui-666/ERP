            ...
            <div className="flex items-center gap-1 text-xs">
              {['已对接', '资料收集', '审核', '制作', '交付'].map((step, i) => {
                const statusMap: Record<string, number> = {
                  PENDING_CONNECTION: -1, CONNECTED: 0, COLLECTING_DOCS: 1,
                  PENDING_REVIEW: 1, UNDER_REVIEW: 2, MAKING_MATERIALS: 3,
                  PENDING_DELIVERY: 3, DELIVERED: 4, APPROVED: 5, REJECTED: 5, PARTIAL: 5,
                }
                ...
                return (
                  <div key={step} className="flex items-center gap-1">
                    ...
                    {i < 4 && (
                      <div className={`h-px w-6 ${i < currentIndex ? 'bg-[var(--color-primary)]' : 'bg-white/10'}`} />
                    )}
                  </div>
                )
              })}

  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-6">
      ...
      {!user ? (
        ...
        <div>
          <button onClick={() => { setResult(null); setSelected(null) }} className="mb-4 text-[13px] text-glass-primary hover:underline">← 返回</button>
          <GlassCard intensity="medium" className="p-5">
            <h2 className="mb-4 text-[16px] font-semibold text-glass-text-primary">生成结果</h2>
            <div className="whitespace-pre-wrap rounded-glass-sm bg-glass-muted/10 p-4 text-[14px] leading-relaxed text-glass-text-primary">{result}</div>
        ...
        <div>
          <button onClick={() => setSelected(null)} className="mb-4 text-[13px] text-glass-primary hover:underline">← 返回列表</button>
          <GlassCard intensity="medium" className="p-5">
            <h2 className="mb-1 text-[16px] font-semibold text-glass-text-primary">{selected.name}</h2>
            <p className="mb-4 text-[12px] text-glass-text-muted/60">{TYPE_LABELS[selected.type] || selected.type} · {selected.language === 'zh' ? '中文' : '英文'}</p>
        ...
        <div className="space-y-4">
          {Object.entries(grouped).map(([type, temps]) => (
            <div key={type}>
              <h3 className="mb-2 text-[14px] font-semibold text-glass-text-primary">{TYPE_LABELS[type] || type}</h3>
                ...
                {temps.map(t => (
                  <GlassCard key={t.id} intensity="light" hover className="cursor-pointer p-4" onClick={() => { setSelected(t); setFormData({}); setResult(null) }}>
                    <div className="flex items-center justify-between">
                      <div><p className="text-[14px] font-medium text-glass-text-primary">{t.name}</p><p className="text-[11px] text-glass-text-muted/60">{t.language === 'zh' ? '中文' : '英文'} · {t.fields.length} 个字段</p></div>
                      <span className="text-[13px] text-glass-primary">生成 →</span>
                    </div>
                  </GlassCard>
                ))}
              </div>

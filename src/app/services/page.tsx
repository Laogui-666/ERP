  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-6">
      ...
      <div className="mb-5 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-[18px] font-semibold text-[var(--color-text-primary)]">签证服务</h1>
          <p className="text-[12px] text-[var(--color-text-secondary)]">覆盖 50+ 国家，专业办理</p>
        </div>
      </div>
      ...
      <div className="mb-4">
        <div className="relative">
          <svg className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--color-text-placeholder)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <input
            ...
            className="w-full rounded-xl border border-white/[0.08] bg-white/[0.05] py-2.5 pl-10 pr-4 text-[14px] text-[var(--color-text-primary)] backdrop-blur-sm placeholder:text-[var(--color-text-placeholder)] focus:border-[var(--color-primary)]/40 focus:outline-none focus:shadow-[0_0_0_3px_rgba(124,141,166,0.1)] transition-all"
          />
        </div>
      </div>
            ...
            <button
              ...
              className={cn(
                ...
                region === r
                  ...
                  : 'bg-white/[0.04] text-[var(--color-text-secondary)] border border-white/[0.06] hover:bg-white/[0.08]'
              )}
            >
              {r}
            </button>
      ...
      {/* 结果计数 */}
      <p className="mb-3 text-[12px] text-[var(--color-text-placeholder)]">
      ...
      <div ref={scrollRef} className="grid grid-cols-2 gap-3">
        {filtered.length === 0 ? (
          <div className="col-span-2 flex flex-col items-center py-16">
            ...
            <p className="mt-2 text-[14px] text-[var(--color-text-secondary)]">未找到相关目的地</p>
            <p className="mt-1 text-[12px] text-[var(--color-text-placeholder)]">试试其他关键词或筛选条件</p>
          </div>
        ) : (
          filtered.map((d, i) => (
            <div
            ...
            >
              ...
              >
                ...
                >
                  ...
                  {/* 出签时间 */}
                  <div className="mt-1.5 flex items-center gap-1 text-[11px] text-[var(--color-text-secondary)]">
                  ...
                  {/* 价格 */}
                  <p className="mt-2 text-[17px] font-bold text-[var(--color-primary)]">
                    ¥{d.price}
                    <span className="ml-0.5 text-[11px] font-normal text-[var(--color-text-secondary)]">起</span>
                  </p>

                  {/* Hover CTA */}
                  <div className="mt-2 flex items-center gap-1 text-[11px] text-[var(--color-primary)] opacity-0 translate-y-1 transition-all duration-300 group-hover:opacity-100 group-hover:translate-y-0">
                    立即办理 →
                  </div>
                </GlassCard>
              </button>

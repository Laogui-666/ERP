...
export function DocumentPanel({ orderId, requirements, userRole, orderStatus, onRefresh }: DocumentPanelProps) {
  ...
  // 角色权限判断（含订单状态约束）
  const isEditableStatus = ['CONNECTED', 'COLLECTING_DOCS', 'PENDING_REVIEW', 'UNDER_REVIEW'].includes(orderStatus)
  const canEdit = ['DOC_COLLECTOR', 'VISA_ADMIN', 'COMPANY_OWNER', 'SUPER_ADMIN'].includes(userRole) && isEditableStatus
  const canReview = ['DOC_COLLECTOR', 'VISA_ADMIN', 'OPERATOR', 'OUTSOURCE'].includes(userRole) && isEditableStatus
  const canUpload = ['CUSTOMER', 'DOC_COLLECTOR', 'VISA_ADMIN'].includes(userRole) && isEditableStatus

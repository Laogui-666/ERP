...
export function AppBottomTab() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-glass-border-light glass-navbar safe-area-bottom md:hidden">
      <div className="mx-auto flex max-w-lg items-center justify-around py-0.5">
        {TABS.map((tab) => {
          const isActive =
            tab.href === '/'
              ? pathname === '/'
          ...
          return (
            <Link
              ...
              href={tab.href}
              className={cn(
                'relative flex flex-col items-center gap-0 px-1 py-0.5 transition-colors duration-200 glass-hover',
                isActive
                  ? 'text-glass-primary'
            ...
            >
              ...
              {isActive && (
                <motion.div 
                  layoutId="bottomTab"
                  className="absolute -bottom-0 h-[1px] w-3 rounded-full bg-glass-primary"
...
function HomeIcon({ active }: { active?: boolean }) {
  return (
    <svg className={`h-4 w-4 ${active ? 'text-glass-primary' : 'text-glass-text-muted'}`} fill={active ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
